...
class Axis( NodePath ):
    ...
    def AddGeometry( self, geom, pos=Point3(0, 0, 0), colour=None, highlight=True, sizeStyle=TRANSLATE , vector='toto'):
        """
        ...
        geom.setPythonTag( 'vector', vector )
    ...
    def AddCollisionSolid( self, collSolid, pos=Point3(0, 0, 0), sizeStyle=TRANSLATE , vector='titi'):
        ...
        collNodePath.setCollideMask(AXIS_COLLISION_MASK)
        collNodePath.setPos( pos )
        collNodePath.setPythonTag( 'sizeStyle', sizeStyle )
        collNodePath.setPythonTag( 'vector', vector )
