...
from ..p3d.geometry import Arc, Line, Cone
...
class Rotation( Base ):
    ...
    def GetAxis( self, collEntry ):
        ...
        for axis in self.axes:
            if axis.vector == CAMERA_VECTOR:
                continue
            if math.fabs( normal.angleDeg( axis.vector ) - 90 ) < 5 :
                return axis
        if self.strict : debug_func()
        #==
    ...
    def OnMouse2Down( self ):
        ...
        if ( hasattr( self, 'collEntry' ) and hasattr( self, 'initCollEntry' ) and 
             ...
             vector = render, self.GetAxisPoint( self.foobar ) - self.getPos() 
        else:
            vector =  render, self.initMousePoint - self.getPos() 
        self.startVec = self.getRelativeVector(vector)
